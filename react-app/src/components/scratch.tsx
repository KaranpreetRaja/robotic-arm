        ...
        return (
          <div className="flex flex-col space-y-2 p-2">
            <div className="flex items-center">
              <span className="font-bold mr-2">Move Joint</span>
              <select 
                className="bg-white text-black rounded px-2 py-1 text-sm"
                value={block.params.joint}
                onChange={(e) => updateBlock(block.id, { ...block.params, joint: e.target.value })}
              >
              ...
              <input 
                ...
                onChange={(e) => updateBlock(block.id, { ...block.params, angle: parseInt(e.target.value) })}
              />
...
export default function ScratchBlocks({ websocketPub, pubConnectionStatus }: { websocketPub: WebSocket | null, pubConnectionStatus: string }) {
  ...
  // Function to get default parameters for each block type
  const getDefaultParams = (type: string) => {
    switch (type) {
      case BLOCK_TYPES.MOVE_JOINT:
        return { joint: '1', angle: 0 };
      case BLOCK_TYPES.MOVE_TO_POSITION:
        return { x: 0, y: 0, z: 0 };
      case BLOCK_TYPES.WAIT:
        return { duration: 1 };
  ...
  // Function to update block parameters
  ...
  // Function to move blocks (for drag and drop)
      ...
      case BLOCK_TYPES.MOVE_JOINT:
        const jointMessage = {
          ...
          message: JSON.stringify({
            orientation: {
              [`value${block.params.joint}`]: parseInt(block.params.angle)
            }
          })
        };
        websocketPub.send(JSON.stringify(jointMessage));
